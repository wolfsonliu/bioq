...
tests/                  离线单测 + opt-in live/contract 测试
...
（见 `src/bioq/main.py`）。脚本按进程退出码分支，而不是解析 stderr 文本
