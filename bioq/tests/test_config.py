...
def test_write_profile_roundtrip_sets_0600(tmp_path):
    from bioq.config import write_profile
...
def test_remove_api_key_drops_key_keeps_url_and_default(tmp_path, monkeypatch):
    monkeypatch.delenv("BIOQ_API_KEY", raising=False)
    from bioq.config import write_profile, remove_api_key
    cfg_file = tmp_path / "config.toml"
    write_profile(cfg_file, profile="prod", gateway_url="https://gw", api_key="k")
...
def test_remove_api_key_noop_when_missing(tmp_path):
    from bioq.config import remove_api_key
    remove_api_key(tmp_path / "nope.toml", "prod")  # must not raise


def test_write_profile_stores_key_id_and_account_id(tmp_path, monkeypatch):
    monkeypatch.delenv("BIOQ_API_KEY", raising=False)
    from bioq.config import write_profile
    cfg_file = tmp_path / "config.toml"
    write_profile(cfg_file, profile="prod", gateway_url="https://gw",
                  api_key="k", key_id="gk_123", account_id="zhiheng")
    cfg = load_config(profile="prod", gateway_url=None, config_path=cfg_file)
    assert cfg.key_id == "gk_123" and cfg.account_id == "zhiheng" and cfg.api_key == "k"


def test_key_id_and_account_id_absent_is_none(tmp_path, monkeypatch):
    monkeypatch.delenv("BIOQ_API_KEY", raising=False)
    from bioq.config import write_profile
    cfg_file = tmp_path / "config.toml"
    write_profile(cfg_file, profile="prod", gateway_url="https://gw", api_key="k")
    cfg = load_config(profile="prod", gateway_url=None, config_path=cfg_file)
    assert cfg.key_id is None and cfg.account_id is None
